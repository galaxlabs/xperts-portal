...
const STATUSES = ["Pending Review", "Approved", "Rejected", "Signed", "Installed", "Live", "Cancelled"];
...
export function LocationsPage() {
  ...
  function toggleSelectAll() {
    ...
    else setSelected(new Set(visibleItems.map((r) => r.name)));
  }
            ...
            <>
              <div className="flex items-center gap-3 py-1.5 text-xs text-muted-foreground border-b">
                <button onClick={toggleSelectAll} className="flex items-center gap-1.5 hover:text-foreground">
                  {selected.size === visibleItems.length ? <CheckSquare className="size-3.5" /> : <Square className="size-3.5" />}
                  Select all
                </button>
