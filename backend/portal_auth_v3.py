...
from cclms.api.auth_v3 import get_active_companies
...
def login():
	...
	companies, _profile = get_active_companies(login_manager.user)
	if not companies:
		login_manager.fail("Portal access is not enabled for an active operator company", user=login_manager.user)
